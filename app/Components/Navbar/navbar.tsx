  ...
  return (
    <nav 
      className={`fixed top-0 left-0 w-full z-100 transition-all duration-300 ${
        scrolled 
        ? "py-3 bg-black/70 backdrop-blur-md border-b border-white/10 shadow-xl" 
        : "py-6 bg-transparent"
      }`}
    >
          ...
          <ul className='flex items-center gap-8'>
            {navLinks.map((link, index) => (
              <li key={index}>
                <a
                  href={link.href}
                  className="text-md lg:text-lg font-medium text-gray-300 hover:text-[#8750f7] transition-colors relative group"
                >
                  {link.label}
